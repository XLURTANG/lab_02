...
as.POSIXct(z, origin = "2021-02-01")
change the class of attribute

```{r}
z <- seq(0, 3600 * 10, by = 3600)
attr(z, which = "class") <- "POSIXct" ## datetime is a class in R represented by POSIXct
z
```
...
```{r}
x[c(TRUE, FALSE)] #will repeat itself
```
